    ...
    <script>
        ...
        map.on('dblclick', function (e) {
            ...
            marker.on('dragend', function (event) {
                ...
                fetch(`/update-pin/${marker.dbId}`, { // بداية التعديل
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            marker.on('mousedown', function () {
                ...
                if (!isDragging) { // فقط إذا لم يكن الدبوس في وضع السحب
                    pressTimer = setTimeout(() => {
                        ...
                        fetch(`/delete-pin/${marker.dbId}`, { // بداية التعديل
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
                            }
        ...
        async function drawShortestPathFromPins() {
            try {
                const response = await fetch('/get-shortest-path-from-pins');
                if (!response.ok) {
                ...
                console.log("بيانات السيرفر:", data); // ✅ هذا السطر مهم
                if (data.error) {
                    alert(data.error);
                    return;
                }

                // const coordinates = data.path.geometry.coordinates;

                // تحويل GeoJSON إلى [lat, lng]
        ...
        // استدعاء الوظيفة تلقائيًا عند تحميل الصفحة
        document.addEventListener('DOMContentLoaded', drawShortestPathFromPins);
